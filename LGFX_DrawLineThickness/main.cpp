...
void drawGrid() {
  ...
  for (int32_t y = -tft.height() * 2; y < tft.height() * 2; y += 20) {
    ...
    y % 100 == 0 ? tft.drawWideLine(x0, y0, x1, y1, 1.0f, color) // slow but with thickness
  ...
  for (int32_t x = -tft.width() * 2; x < tft.width() * 2; x += 20) {
    ...
    float x1       = (x1Pos * cos1 - y1Pos * sin1) + cv.middleX;
    float y1       = (y1Pos * cos1 + x1Pos * sin1) + cv.middleY;
    x % 100 == 0 ? tft.drawWideLine(x0, y0, x1, y1, 1.0f, color) // slow but with thickness
                       : tft.drawSmoothLine(x0, y0, x1, y1, color)     // just antialiased
        ;
  }
}
...
void runRandomLinesDemo() {
  for (int i = 0; i < 256; i++) {
    ...
    switch (i++ % 5) {
    case 0:
      tft.drawWedgeLine(v.x0, v.y0, v.x1, v.y1, v.thickness0, v.thickness1, v.gradient888);
      break;
    case 1:
      tft.drawWedgeLine(v.x0, v.y0, v.x1, v.y1, v.thickness0, v.thickness1, v.colorstart);
      break;
    case 2:
      tft.drawWedgeLine(v.x0, v.y0, v.x1, v.y1, v.thickness0, v.thickness1, v.color24start);
      break;
    case 3:
      tft.drawWedgeLine(v.x0, v.y0, v.x1, v.y1, v.thickness0, v.thickness1, v.color16start);
      break;
    case 4:
      tft.drawWedgeLine(v.x0, v.y0, v.x1, v.y1, v.thickness0, v.thickness1, v.color8start);
      break;
    }
  }
}
...
void runGradientDemo() {
  ...
  for (int i = 0; i < 360; i += 15) {
    ...
    switch ((i / 15) % 8) {
    case 0:
      tft.drawWedgeLine(x0, y0, x1, y1, cv.thick1, cv.thick3, heatMapGradient);
      break;
    case 1:
      tft.drawWedgeLine(x0, y0, x1, y1, cv.thick1, cv.thick3, gradient888);
      break;
    case 2:
      tft.drawWedgeLine(x1, y1, x0, y0, cv.thick3, cv.thick1, gradient888);
      break;
    case 3:
      tft.drawWedgeLine(x1, y1, x0, y0, cv.thick3, cv.thick1, heatMapGradient);
      break;
    case 4:
      tft.drawWedgeLine(x0, y0, x1, y1, cv.thick2, cv.thick3, 0xff0000u);
      break; // red 24bits
    case 5:
      tft.drawWedgeLine(x0, y0, x1, y1, cv.thick2, cv.thick3, (lgfx::rgb332_t)0x82);
      break; // purple 8bits
    case 6:
      tft.drawWedgeLine(x0, y0, x1, y1, cv.thick2, cv.thick3, TFT_ORANGE);
      break; // named colors default to 16bits rgb565_t
    case 7:
      tft.drawWedgeLine(x0, y0, x1, y1, cv.thick2, cv.thick3, RGBColor{0x00, 0xff, 0x00});
      break; // green
    }
